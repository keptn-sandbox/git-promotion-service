...
WORKDIR /go/src/github.com/keptn/keptn/promotion-service
...
RUN GOOS=linux go build -ldflags '-linkmode=external' $BUILDFLAGS -v -o promotion-service
...
LABEL org.opencontainers.image.source="https://github.com/keptn/keptn" \
    ...
    org.opencontainers.image.title="Keptn Promotion Service" \
...
COPY --from=builder-base /go/src/github.com/keptn/keptn/promotion-service/promotion-service /promotion-service
EXPOSE 8080
